...
public class MapAreaEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (totalChanceInGrass != 100 && totalChanceInGrass != -1){
            EditorGUILayout.HelpBox($"The total chance percentage of pokemons in grass is {totalChanceInGrass} and not 100", MessageType.Error);
        }

        if (totalChanceInWater != 100 && totalChanceInWater != -1){
            EditorGUILayout.HelpBox($"The total chance percentage of pokemons in water is {totalChanceInWater} and not 100", MessageType.Error);
        }
    }
}
