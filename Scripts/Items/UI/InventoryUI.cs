...
public class InventoryUI : MonoBehaviour
{
    ...
    public void HandleUpdate(Action onBack, Action<ItemBase> onItemUsed = null)
    {
        ...
        if (state == InventoryUIState.ItemSelection)
        {
            ...
            // Rotating the selection from last-to-first and first-to-last
            if (selectedCategory > Inventory.ItemCategories.Count - 1)
                selectedCategory = 0;
            else if (selectedCategory < 0)
                selectedCategory = Inventory.ItemCategories.Count - 1;

            // Clamping the selection between first-to-last
    ...
    IEnumerator HandleTmItems()
    {
        ...
        else
        {
            ...
            yield return DialogManager.Instance.ShowDialogText($"{pokemon.Base.Name} cannot learn more than {PokemonBase.MaxNumOfMoves} moves");
